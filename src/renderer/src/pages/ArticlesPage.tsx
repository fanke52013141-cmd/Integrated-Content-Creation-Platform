...
export function ArticlesPage({ accounts, providers, currentAccountId, onNavigate, showToast }: ArticlesPageProps): React.JSX.Element {
  ...
  async function remove(): Promise<void> { if (!selected || !window.confirm('确定删除这篇成稿及其全部本地版本吗？')) return; try { await window.moliu.articles.remove(selected.id); await refresh(); showToast({ type: 'success', message: '成稿已删除' }) } catch (error) { showToast({ type: 'error', message: errorMessage(error) }) } }
