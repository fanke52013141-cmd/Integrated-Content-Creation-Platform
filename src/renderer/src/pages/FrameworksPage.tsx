  ...
  return <div className="page frameworks-page">
    ...
    <section className="framework-composer">
      ...
      <div className="framework-compose-grid">
        <label className="field"><span>框架模板</span><select name="templateId" autoComplete="off" value={templateId} onChange={(event) => setTemplateId(event.target.value)}>{templates.map((template) => <option key={template.id} value={template.id}>{template.name}{template.isDefault ? '（默认）' : ''}</option>)}</select></label>
        <label className="field"><span>账号定位（可选）</span><select name="accountId" autoComplete="off" value={accountId} onChange={(event) => setAccountId(event.target.value)}><option value="">不使用账号定位</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}{account.status === 'draft' ? ' · 草稿' : ''}</option>)}</select></label>
        <label className="field"><span>选题（可选）</span><select name="topicId" autoComplete="off" value={topicId} onChange={(event) => setTopicId(event.target.value)}><option value="">不关联选题</option>{topics.map((topic) => <option key={topic.id} value={topic.id}>{topic.fields['选题主题'] || topic.seedKeyword}</option>)}</select></label>
      ...
      <div className="framework-material-picker"><div><strong><FolderHeart size={16} />引用素材（可选）</strong><span>仅注入保存的摘要；图片仅作视觉参考，不会参与正文事实生成。</span></div><button className="button ghost compact" onClick={() => onNavigate('materials')}>管理素材</button><div className="framework-material-options">{usableMaterials.length ? usableMaterials.slice(0, 12).map((material) => <label key={material.id} className={materialIds.has(material.id) ? 'selected' : ''}><input type="checkbox" name="materialId" autoComplete="off" checked={materialIds.has(material.id)} onChange={(event) => setMaterialIds((current) => { const next = new Set(current); event.target.checked ? next.add(material.id) : next.delete(material.id); return next })} /><span>{material.title}</span><small>{material.kind === 'web' ? '网页 Summary' : '文字'}</small></label>) : <p>素材集合为空也不影响生成。</p>}</div></div>
      <footer><label className="field framework-count"><span>生成数量</span><select name="frameworkCount" autoComplete="off" value={count} onChange={(event) => setCount(Number(event.target.value))}>{[1, 2, 3].map((value) => <option key={value} value={value}>{value} 个独立框架</option>)}</select></label><button className="button ghost compact" onClick={() => setTemplateEditor('new')}><Plus size={14} />新建模板</button><span>每个候选会独立调用模型，保存为可编辑草稿。</span><button className="button primary" disabled={generating || !availableModels.length} onClick={() => void generate()}>{generating ? <LoaderCircle size={16} className="spin" /> : <Sparkles size={16} />}{generating ? '正在生成…' : '生成内容框架'}</button></footer>
    </section>

    <section className="framework-wall"><header><div><span className="eyebrow">OUTLINE DRAFTS</span><h3>内容框架 <small>{frameworks.length}</small></h3></div><span>最新生成排在前面；保存编辑会形成新的版本。</span></header>{frameworks.length ? <div className="framework-card-grid"><VirtualList items={frameworks} estimateSize={() => 120} renderItem={(framework) => <FrameworkCard key={framework.id} framework={framework} onEdit={() => setEditing(framework)} onToggleLock={() => void toggleLocked(framework)} onRemove={() => void remove(framework)} />} /></div> : <div className="large-empty"><WandSparkles size={34} /><h3>还没有内容框架</h3><p>选择一个选题或输入手动主题，生成第一批可继续打磨的文章提纲。</p></div>}</section>
