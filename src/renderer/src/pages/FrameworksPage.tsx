...
export function FrameworksPage({
  ...
  async function remove(framework: Framework): Promise<void> {
    if (!window.confirm('确定删除这个内容框架吗？其本地版本记录会一并删除。')) return
  ...
  return <div className="page frameworks-page">
    <section className="page-intro frameworks-intro">
      <div><span className="eyebrow"><WandSparkles size={14} /> OUTLINE STUDIO</span><h2>先把文章想清楚，再开始写。</h2><p>选题、账号定位与素材均可选。素材只以摘要参与推理；未选择素材时，框架仍可独立生成。</p></div>
    ...
    <section className="framework-composer">
      <header><div><span className="eyebrow">GENERATE OUTLINES</span><h3>发起一个框架批次</h3></div><span>{selectedTemplate ? `${selectedTemplate.sections.length} 个章节` : '选择模板'}</span></header>
      <div className="framework-compose-grid">
        <label className="field"><span>框架模板</span><select value={templateId} onChange={(event) => setTemplateId(event.target.value)}>{templates.map((template) => <option key={template.id} value={template.id}>{template.name}{template.isDefault ? '（默认）' : ''}</option>)}</select></label>
        <label className="field"><span>账号定位（可选）</span><select value={accountId} onChange={(event) => setAccountId(event.target.value)}><option value="">不使用账号定位</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}{account.status === 'draft' ? ' · 草稿' : ''}</option>)}</select></label>
        <label className="field"><span>选题（可选）</span><select value={topicId} onChange={(event) => setTopicId(event.target.value)}><option value="">不关联选题</option>{topics.map((topic) => <option key={topic.id} value={topic.id}>{topic.fields['选题主题'] || topic.seedKeyword}</option>)}</select></label>
        <label className="field"><span>连接与模型</span><select value={modelTarget} onChange={(event) => setModelTarget(event.target.value)}><option value="">选择模型</option>{availableModels.map(({ provider, model }) => <option key={`${provider.id}:${model.modelId}`} value={encodeModelTarget(provider.id, model.modelId)}>{model.displayName} · {provider.displayName}</option>)}</select></label>
      </div>
      <label className="field framework-topic-field"><span>补充主题（未选选题时必填）</span><textarea rows={2} maxLength={2000} value={manualTopic} onChange={(event) => setManualTopic(event.target.value)} placeholder="例如：为什么创作者应该先写框架，再写正文？" /></label>
      <div className="framework-material-picker"><div><strong><FolderHeart size={16} />引用素材（可选）</strong><span>仅注入保存的摘要；图片仅作视觉参考，不会参与正文事实生成。</span></div><button className="button ghost compact" onClick={() => onNavigate('materials')}>管理素材</button><div className="framework-material-options">{usableMaterials.length ? usableMaterials.slice(0, 12).map((material) => <label key={material.id} className={materialIds.has(material.id) ? 'selected' : ''}><input type="checkbox" checked={materialIds.has(material.id)} onChange={(event) => setMaterialIds((current) => { const next = new Set(current); event.target.checked ? next.add(material.id) : next.delete(material.id); return next })} /><span>{material.title}</span><small>{material.kind === 'web' ? '网页 Summary' : '文字'}</small></label>) : <p>素材集合为空也不影响生成。</p>}</div></div>
      <footer><label className="field framework-count"><span>生成数量</span><select value={count} onChange={(event) => setCount(Number(event.target.value))}>{[1, 2, 3].map((value) => <option key={value} value={value}>{value} 个独立框架</option>)}</select></label><button className="button ghost compact" onClick={() => setTemplateEditor('new')}><Plus size={14} />新建模板</button><span>每个候选会独立调用模型，保存为可编辑草稿。</span><button className="button primary" disabled={generating || !availableModels.length} onClick={() => void generate()}>{generating ? <LoaderCircle size={16} className="spin" /> : <Sparkles size={16} />}{generating ? '正在生成…' : '生成内容框架'}</button></footer>
    </section>

    <section className="framework-wall"><header><div><span className="eyebrow">OUTLINE DRAFTS</span><h3>内容框架 <small>{frameworks.length}</small></h3></div><span>最新生成排在前面；保存编辑会形成新的版本。</span></header>{frameworks.length ? <div className="framework-card-grid">{frameworks.map((framework) => <FrameworkCard key={framework.id} framework={framework} onEdit={() => setEditing(framework)} onToggleLock={() => void toggleLocked(framework)} onRemove={() => void remove(framework)} />)}</div> : <div className="large-empty"><WandSparkles size={34} /><h3>还没有内容框架</h3><p>选择一个选题或输入手动主题，生成第一批可继续打磨的文章提纲。</p></div>}</section>
...
function FrameworkEditor({ framework, onClose, onSaved, showToast }: { framework: Framework; onClose(): void; onSaved(): Promise<void>; showToast(toast: ToastState): void }): React.JSX.Element {
  ...
  return <div className="modal-overlay"><section className="framework-editor-dialog" role="dialog" aria-modal="true"><header><div><span className="eyebrow">EDIT OUTLINE · V{framework.versionCount + 1}</span><h2>打磨内容框架</h2><p>保存会保留当前版本，并创建新的本地版本记录。</p></div><button className="icon-button" onClick={onClose}><X size={18} /></button></header><div className="framework-editor-sections">{sections.map((section, index) => <label className="field" key={section.name}><span>{section.name}</span><textarea rows={4} value={section.content} onChange={(event) => setSections((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, content: event.target.value } : item))} /></label>)}</div><footer><span>{framework.materialIds.length} 条素材引用 · {framework.status === 'locked' ? '锁定状态下编辑会创建新版本' : '草稿可继续编辑'}</span><button className="button secondary" onClick={onClose}>取消</button><button className="button primary" disabled={saving || sections.some((section) => !section.content.trim())} onClick={() => void save()}>{saving ? <LoaderCircle size={15} className="spin" /> : <Check size={15} />}保存新版本</button></footer></section></div>
}
